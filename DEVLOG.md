...
- Reconciling that against all 10 real G702/G703 files and the actual bank checks turned up a genuine gap on Draw 1 (its own document's retainage didn't match what was stored, a pre-existing $371.55 difference unrelated to the elevator) — corrected `amount_requested`/`retainage_held` on Draws 1 and 8 against their own documents plus the elevator surcharge, leaving `amount_paid` as the real cash received. Also chased what looked like a $378.23 misallocation between Draws 3 and 4, but that was a false positive from comparing *cumulative* totals across files rather than each draw's own numbers — Draws 3 and 4 already matched their own documents exactly (requested = paid), so left as-is. (Draws 1 & 2's G702 files also had a mislabeled header — "7655 San Pedro, LLC" instead of Hi Point, clearly a copied template — but the checks' memo lines confirmed the actual payments were correct.) Portfolio retainage lands at $298,816.94 — the real $291,810.30 flat-5% total plus $7,006.64 in deliberate elevator surcharge, traceable to the cent.
