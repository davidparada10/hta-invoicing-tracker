...
- Reconciling that surfaced two more issues on Hi Point, found by reading all 10 real G702/G703 files and the actual bank checks: Draw 1's own document didn't match what was stored (a pre-existing $371.55 gap, unrelated to the elevator), and $378.23 had been cut into the wrong draw at the Draw 3/4 boundary — their sums matched, just misallocated. (Draws 1 & 2's G702 files also had a mislabeled header — "7655 San Pedro, LLC" instead of Hi Point, clearly a copied template — but the checks' memo lines confirmed the actual payments were correct.) Corrected `amount_requested`/`retainage_held` on Draws 1, 3, 4, and 8 against the real documents plus the elevator surcharge; `amount_paid` left untouched as the real cash received throughout, so two draws now show a small paid/requested gap that reflects what actually happened rather than a rewritten history. Portfolio retainage lands at $298,816.93 — the real $291,810.30 flat-5% total plus $7,006.64 in deliberate elevator surcharge, traceable to the cent.
