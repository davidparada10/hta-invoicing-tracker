...
## 2026-09-21
- Added `retention_rate_override` to budget lines so one line item can retain at its own rate regardless of the draw's selected uniform rate — same pattern as the existing `retention_exempt` flag (a checkbox for 0%), but for a nonzero custom rate. Set 1532 Hi Point's Elevator line to 10% (vs. the project's standard 5%) and corrected the two already-paid draws that billed it (Draws 1 & 8) — verified against the real G702s first, which confirmed the previous-application chain (Draw 8→9→10) ties out correctly, but also that the certified paperwork itself showed flat 5% with no elevator carve-out. Corrected `amount_requested`/`retainage_held` on both to reflect true 10%; left `amount_paid` as the real cash received, so the draws now show paid slightly ahead of requested rather than rewriting what actually happened.
